...
We evaulated each model based on sequence level class assignments. Model performance for each model in shown in \tableref{tab:table2}. TF-IDF achieved AUC of 0.95 and accuracy f 0.84. The 20 words with the highest correlation coefficients using TF-IDF word vectorization are shown in Appendix B. While TF-IDF was able to identify the presence of a keyword or always pattern in a sequence, it was unable to leverage the context around each keyword match, which occasionally would negate the presence of CI. This led to the model over-zealously predicting sequences as demonstrating CI. ClincalBERT was able to overcome these issues due to its high complexity transformer architecture, and achieved AUC of 0.98 and accuracy of 0.93.  
